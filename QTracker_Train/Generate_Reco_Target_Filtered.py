...
while(total_entries<5000000):
    try:
        hits, drift, kinematics = generate_e906(0)
        tf.keras.backend.clear_session()
        print("Loaded events")
        model = keras.models.load_model('event_filter')
        ...
        if(len(hits>0)):
            ...
            model=tf.keras.models.load_model('Reconstruction_All')
            ...
            model=tf.keras.models.load_model('Vertexing_All')
            ...
            model = tf.keras.models.load_model('Track_Finder_Z')
            predictions = (np.round(model.predict(hits,verbose=0)*max_ele)).astype(int)
            ...
            model = tf.keras.models.load_model('Track_Finder_Target')
            predictions = (np.round(model.predict(hits,verbose=0)*max_ele)).astype(int)
            ...
            model=tf.keras.models.load_model('Reconstruction_Target')
            ...
            t = np.column_stack((all_vtx_track,z_vtx_track,target_vtx_track))
            
            target_dump_input = np.column_stack((reco_kinematics,t.reshape((len(t),(204*2)))))
            
            tf.keras.backend.clear_session()
            tf.compat.v1.reset_default_graph()
            model=tf.keras.models.load_model('target_dump_filter')
            ...
            np.save('target_tracks_train.npy',np.concatenate(tracks, axis=0))
            np.save('target_kinematics_train.npy',np.concatenate(all_predictions, axis=0))
            np.save('target_truth_train.npy',np.concatenate(truth, axis=0))
...
while(total_entries<500000):
    try:
        hits, drift, kinematics = generate_e906(1)
        tf.keras.backend.clear_session()
        print("Loaded events")
        model = keras.models.load_model('event_filter')
        probability_model = tf.keras.Sequential([model, tf.keras.layers.Softmax()])
        ...
        if(len(hits>0)):
            ...
            np.save('target_tracks_val.npy',np.concatenate(tracks, axis=0))
            np.save('target_kinematics_val.npy',np.concatenate(all_predictions, axis=0))
            np.save('target_truth_val.npy',np.concatenate(truth, axis=0))

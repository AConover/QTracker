...
while n_train < 1e7:
    ...
    val_predictions = (np.round(track_finder_model.predict(valin, verbose=0) * max_ele)).astype(int)
    track_val = evaluate_finder(valin, valdrift, val_predictions)
    results_val = calc_mismatches(track_val)
    val_mask = ((results_val[0::4] < 2) & (results_val[1::4] < 2) & (results_val[2::4] < 3) & (results_val[3::4] < 3)).all(axis=0)
    
    train_predictions = (np.round(track_finder_model.predict(trainin, verbose=0) * max_ele)).astype(int)

...
while(total_entries<5000000):
    try:
        ...
        if(len(hits>0)):
            ...
            np.save('dump_tracks_train.npy',np.concatenate(tracks, axis=0))
            np.save('dump_kinematics_train.npy',np.concatenate(all_predictions, axis=0))
            np.save('dump_truth_train.npy',np.concatenate(truth, axis=0))
...
while(total_entries<500000):
    try:
        ...
        if(len(hits>0)):
            ...
            np.save('dump_tracks_val.npy',np.concatenate(tracks, axis=0))
            np.save('dump_kinematics_val.npy',np.concatenate(all_predictions, axis=0))
            np.save('dump_truth_val.npy',np.concatenate(truth, axis=0))

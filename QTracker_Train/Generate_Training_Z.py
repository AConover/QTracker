...
def generate_e906(n_events,tvt):
    ...
    if(tvt=="Train"):
        targettree = uproot.open("Root_Files/Z_Train_QA.root:QA_ana")
    if(tvt=="Val"):
        targettree = uproot.open("Root_Files/Z_Val_QA.root:QA_ana")
    if(tvt=="Test"):
        targettree = uproot.open("Root_Files/Z_Test_QA.root:QA_ana")
...
while(n_train<1e7):
    ...
    print(n_train)
    np.save('Z_Train_In.npy',np.concatenate((train_input)))
    np.save('Z_Val_In.npy',np.concatenate((val_input)))
    np.save('Z_Train_Out.npy',np.concatenate((train_kinematics)))
    np.save('Z_Val_Out.npy',np.concatenate((val_kinematics)))

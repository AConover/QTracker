...
job5_id=$(sbatch --dependency=afterok:$job2_id Training_Jobscripts/jobscript_Kin_All.sh | awk '{print $4}')

# Submit the Z-Vertex Kinematic and Vertex Reconstruction job once the Z-Vertex Finder is done training.
job6_id=$(sbatch --dependency=afterok:$job3_id Training_Jobscripts/jobscript_Kin_Z.sh | awk '{print $4}')

# Submit the Target-Vertex Kinematic Reconstruction job once the Target-Vertex Finder is done training.
job7_id=$(sbatch --dependency=afterok:$job4_id Training_Jobscripts/jobscript_Kin_Target.sh | awk '{print $4}')

# Submit the target-dump filter training once everything else is done.
sbatch --dependency=afterok:$job5_id:$job6_id:$job7_id Training_Jobscripts/jobscript_Target_Dump_Filter.sh
